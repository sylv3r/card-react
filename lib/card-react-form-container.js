(function() {
  ...
  CardReactFormContainer = React.createClass({
    ...
    traverseChildrenAndRegisterInputs: function(children) {
      ...
      return React.Children.map(children, (function(_this) {
        return function(child) {
          var inputsValidationClass, newClassName, inputNameAttribute;
          if (typeof child !== 'object' || child === null) {
            return child;
          }
          if (child.props && child.props.id) {
            inputNameAttribute = child.props[_this.props.formInputsNameAttribute];
            newClassName = child.props.className;
            inputsValidationClass = _this.state.inputsValidationClass[inputNameAttribute];
            if (child.ref) {
              _this.inputsRefs[inputNameAttribute] = child.ref;
            } else {
              _this.inputsRefs[inputNameAttribute] = "react-card-input-" + inputNameAttribute;
            }
            ...
            return React.cloneElement(child, {
              ...
              ref: _this.inputsRefs[inputNameAttribute],
              defaultValue: _this.inputsValues[inputNameAttribute],
    ...
    inputOnKeyUp: function(event) {
      this.input = event.target[this.props.formInputsNameAttribute];
    ...
    inputOnFocus: function(event) {
      this.focusedInput = event.target[this.props.formInputsNameAttribute];
